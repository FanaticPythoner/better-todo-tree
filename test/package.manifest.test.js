...
function collectLocalizationKeys( value, keys )
{
    ...
    {
        value.forEach( function( entry ) { collectLocalizationKeys( entry, keys ); } );
    }
    else if( value && typeof ( value ) === 'object' )
    {
        Object.keys( value ).forEach( function( key ) { collectLocalizationKeys( value[ key ], keys ); } );
    }
...
QUnit.test( 'every manifest localization key exists in both language bundles', function( assert )
{
    var keys = Array.from( collectLocalizationKeys( readPackageJson() ) ).sort();
    ...
    keys.forEach( function( key )
    {
        ...
        assert.equal( typeof ( chinese[ key ] ), 'string', 'Simplified Chinese: ' + key );
    } );
} );

QUnit.test( 'simplified Chinese localizes every manifest string except product names', function( assert )
{
    var keys = Array.from( collectLocalizationKeys( readPackageJson() ) ).sort();
    var english = readPackageNls( 'package.nls.json' );
    var chinese = readPackageNls( 'package.nls.zh-cn.json' );
    var productNameKeys = new Set( [
        'better-todo-tree.configuration.ripgrep'
    ] );
    var untranslated = keys.filter( function( key )
    {
        return productNameKeys.has( key ) !== true && english[ key ] === chinese[ key ];
    } );
...
QUnit.test( 'prepublish builds are deterministic and codicon updates are explicit', function( assert )
{
    ...
    assert.equal( scripts[ 'vscode:prepublish' ],
        'node ./scripts/sync-settings-aliases.js --check && webpack --mode production' );
    assert.equal( scripts[ 'codicons:update' ], 'python3 ./scripts/build-codicon-assets.py' );
    assert.equal( scripts[ 'vscode:prepublish' ].indexOf( 'build-codicon-assets' ), -1 );
} );

QUnit.test( 'generated codicon table stores aliases instead of codepoints', function( assert )
{
    var codiconNames = Object.keys( require( '../src/codiconAliases.json' ) );
...
QUnit.test( 'issue #110 highlight colour scheme is opt-in', function( assert )
{
    var currentUseColourScheme = getConfigurationProperty( 'better-todo-tree.highlights.useColourScheme' );
    var readme = readRepositoryFile( 'README.md' ).toString( 'utf8' );

    assert.strictEqual( currentUseColourScheme.default, false );
    assert.ok( readme.indexOf( '**better-todo-tree.highlights.foregroundColourScheme**' ) !== -1 );
    assert.strictEqual( readme.indexOf( 'highlights.foreroundColourScheme' ), -1 );
} );
...
QUnit.test( 'ripgrep executable setting documents packaged binary behavior', function( assert )
{
    ...
    assert.equal( englishNls[ 'better-todo-tree.configuration.ripgrep.ripgrep.markdownDescription' ],
        'Custom ripgrep executable path. Empty value uses the packaged ripgrep binary.' );
} );

QUnit.test( 'release targets match the Node and native-ripgrep runtime contract', function( assert )
{
    var packageJson = readPackageJson();
    var targets = require( '../scripts/release/targets.json' );
    var readme = readRepositoryFile( 'README.md' ).toString( 'utf8' );

    assert.equal( packageJson.main, './dist/extension' );
    assert.strictEqual( packageJson.browser, undefined );
    assert.equal( targets.length, 9 );
    assert.strictEqual( targets.includes( 'web' ), false );
    assert.ok( readme.indexOf( 'Browser-hosted VS Code is excluded' ) !== -1 );
} );

QUnit.test( 'streaming ripgrep output exposes no inert buffer-size setting', function( assert )
{
    var configurationProperties = readPackageJson().contributes.configuration.reduce( function( properties, group )
    {
        return Object.assign( properties, group.properties || {} );
    }, {} );
    var readme = readRepositoryFile( 'README.md' ).toString( 'utf8' );
    var packageNls = readPackageNls( 'package.nls.json' );
    var packageNlsZhCn = readPackageNls( 'package.nls.zh-cn.json' );
    var extensionSource = readRepositoryFile( 'src/extension.js' ).toString( 'utf8' );

    assert.false( Object.prototype.hasOwnProperty.call( configurationProperties, 'better-todo-tree.ripgrep.ripgrepMaxBuffer' ) );
    assert.equal( configurationProperties[ 'todo-tree.ripgrep.ripgrepMaxBuffer' ].type, 'integer' );
    assert.strictEqual( packageNls[ 'better-todo-tree.configuration.ripgrep.ripgrepMaxBuffer.markdownDescription' ], undefined );
    assert.strictEqual( packageNlsZhCn[ 'better-todo-tree.configuration.ripgrep.ripgrepMaxBuffer.markdownDescription' ], undefined );
    assert.strictEqual( extensionSource.indexOf( 'ripgrepMaxBuffer' ), -1 );
} );

QUnit.test( 'Todo Tree aliases share the current schema and explicit palette opt-in', function( assert )
{
    var synchronize = require( '../scripts/sync-settings-aliases.js' ).synchronize;
    var manifest = readPackageJson();
    assert.deepEqual( synchronize( JSON.parse( JSON.stringify( manifest ) ) ), manifest );
    assert.deepEqual( getConfigurationProperty( 'todo-tree.general.tags' ),
        getConfigurationProperty( 'better-todo-tree.general.tags' ) );
    assert.strictEqual( getConfigurationProperty( 'todo-tree.highlights.useColourScheme' ).default, false );
} );

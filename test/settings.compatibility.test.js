...
QUnit.test( 'manifest exposes current and legacy settings namespaces together', function( assert )
{
    ...
    assert.equal( currentSettings.length, 73 );
    assert.equal( legacySettings.length, 73 );
    ...
    assert.ok( legacySettings.indexOf( 'todo-tree.general.showScanningProgress' ) !== -1 );
    assert.ok( legacySettings.indexOf( 'todo-tree.languages.customPatterns' ) !== -1 );
    assert.ok( legacySettings.indexOf( 'todo-tree.languages.embeddedDocuments' ) !== -1 );
} );

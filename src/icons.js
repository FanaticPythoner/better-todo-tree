...
var codiconAliases = require( './codiconAliases.json' );
...
function writeIconFile( context, svg )
{
    var digest = crypto.createHash( 'sha256' ).update( svg ).digest( 'hex' );
    var filePath = path.join( context.globalStorageUri.fsPath, 'todo-' + digest + '.svg' );
...
{
    if( !Object.prototype.hasOwnProperty.call( codiconAliases, iconName ) )
    {
        throw new TypeError( 'Unknown gutter codicon: ' + iconName );
    }
    var template = require( './codiconGlyphs.json' )[ codiconAliases[ iconName ] ];
    var encodedColour = [ [ '&', '&amp;' ], [ '"', '&quot;' ], [ '<', '&lt;' ], [ '>', '&gt;' ] ]
        .reduce( ( value, pair ) => value.split( pair[ 0 ] ).join( pair[ 1 ] ), colour );
    var svg = template.split( 'currentColor' ).join( encodedColour );
    return writeIconFile( context, svg );
}

function getFileBackedIcon( context, tag, debug )
{
    ensureStorageDirectory( context, debug );
    ...
    {
        darkIconPath = createOcticonFile( context, iconName, colour );
        lightIconPath = darkIconPath;
    }
    else if( utils.isHexColour( colour ) || utils.isRgbColour( colour ) || utils.isNamedColour( colour ) )
    {
        darkIconPath = createColourIconFile( context, colour );
        lightIconPath = darkIconPath;
    }
...
function getGutterIcon( context, tag, debug )
{
    return getFileBackedIcon( context, tag, debug );
}
...
{
    ensuredStorageDirectories.clear();
    generatedIconFiles.clear();
    fileBackedIconCache.clear();
}
    ...
    function checkIcon( setting )
    {
        ...
        if( icon !== undefined )
        {
            if( utils.isCodicon( icon ) )
            {
                ...
                if( !Object.prototype.hasOwnProperty.call( codiconAliases, codicon ) )
                {
                    invalidIcons.push( setting + '.icon(' + icon + ')' );
                }
            }
            else if( !Object.prototype.hasOwnProperty.call( octicons, icon ) &&
                icon !== 'todo-tree' &&
...
module.exports.getTreeIcon = getTreeIcon;
module.exports.getGutterIcon = getGutterIcon;
module.exports.getIcon = getTreeIcon;

...
function createTodoTreeIcon( context, iconName, colour )
{
    ...
    writeIconFile( filePath, svg );
    return filePath;
}

function createOcticonFile( context, iconName, colour )
{
    var resolvedIconName = octicons[ iconName ] ? iconName : "check";
    var filePath = path.join( context.globalStorageUri.fsPath, "todo-" + resolvedIconName + "-" + compactColourName( colour ) + ".svg" );
    var svg = "<?xml version=\"1.0\" encoding=\"iso-8859-1\"?>\n" +
        octicons[ resolvedIconName ].toSVG( { "xmlns": "http://www.w3.org/2000/svg", "fill": colour } );
...
{
    var filePath = path.join( context.globalStorageUri.fsPath, "todo-" + compactColourName( colour ) + ".svg" );
    var svg =
...
function createCodiconFile( context, iconName, colour )
{
    ...
    var svg =
        ...
        "<text x=\"8\" y=\"11\" text-anchor=\"middle\" font-size=\"8\" font-family=\"Verdana,Arial,sans-serif\" fill=\"" + foreground + "\">" + glyph + "</text>" +
        "</svg>";
